  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if y is None:
      return scores
    	...
    	y_hot = np.zeros((X.shape[0], np.max(y)+1))
    	y_hot[np.arange(X.shape[0]), y] = 1
    ...
    # output layer
    def grad_scores(q):
        dscores = stable_softmax(q)
        dscores[np.arange(X.shape[0]),y] -= 1
        dscores /= X.shape[0]
        return dscores
    
    dscore = grad_scores(scores)  # partial derivative of loss wrt. the logits (dL/dz)
    dW2 = np.dot(hidden.T, dscore) + reg*W2    # partial derivative of loss wrt. W2
    ...
    for epoch in xrange(num_epochs):
        ...
        for it in xrange(iterations_per_epoch):
            ...
            for param in self.params:
                self.params[param] = self.params[param] - grads[param] * learning_rate
